...
public class View_PieChart extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < mDataArray.size(); i++) {
			...
			mBgPaints.setColor(Item.getColor());
	...
	public int getColorValue( int Index ) {
		...
		if (Index < 0){
			return ((PieItem)mDataArray.get(0)).getColor();
		} else if (Index >= mDataArray.size()){
			return ((PieItem)mDataArray.get(mDataArray.size()-1)).getColor();
		} else {
			return ((PieItem)mDataArray.get(mDataArray.size()-1)).getColor();
		}
	}
